...
## Relationship: advertising budget and sales {.smaller}
...
tv_model <- lm(sales ~ radio, data=advertising)
cat(sprintf("Sales (1k units) = %.4f %+.4f Radio ($ 1k)\n", 
            tv_model$coefficients["(Intercept)"], 
            tv_model$coefficients["radio"]))
```
...
tv_model <- lm(sales ~ newspaper, data=advertising)
cat(sprintf("Sales (1k units) = %.4f %+.4f Newspaper ($ 1k)\n", 
            tv_model$coefficients["(Intercept)"], 
            tv_model$coefficients["newspaper"]))
```
...
::: {.column width="60%"}
- According to the [Central Limit Theorem](https://statisticsbyjim.com/basics/central-limit-theorem/), the mean of this sampling distribution can be approximated by a Normal distribution.
- We know from Normal theory that 95% of the distribution lies in $[-1.96, 1.96]$
...
::: {.fragment}
- The <mark>standard error</mark> of an estimator reflects how it varies under repeated sampling.
:::
...
- These standard errors can be used to compute <mark>confidence intervals</mark>. 
:::
